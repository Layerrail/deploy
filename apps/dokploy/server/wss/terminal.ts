...
cat /etc/LayerRail Deploy/ssh/auto_generated-LayerRail Deploy-local.pub >> $HOME/.ssh/authorized_keys && \\
echo "✓ LayerRail Deploy SSH key added successfully. Reopen the terminal in LayerRail Deploy to reconnect."
# ----------------------------------------`;

const COMMAND_TO_GRANT_PERMISSION_ACCESS = `
# ----------------------------------------
sudo chown -R $USER:$USER /etc/LayerRail Deploy/ssh
# ----------------------------------------
`;
