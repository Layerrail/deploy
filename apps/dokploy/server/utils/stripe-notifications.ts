import InvoiceNotificationEmail from "@LayerRail Deploy/server/emails/emails/invoice-notification";
import PaymentFailedEmail from "@LayerRail Deploy/server/emails/emails/payment-failed";
import { sendEmail } from "@LayerRail Deploy/server/verification/send-verification-email";
...
export const sendInvoiceEmail = async (
	...
	try {
		...
		if (invoice.invoice_pdf) {
			...
			if (pdfBuffer) {
				attachments.push({
					filename: `LayerRail Deploy-invoice-${invoice.number || invoice.id}.pdf`,
					content: pdfBuffer,
				});
			}
		}
		...
		await sendEmail({
			email: admin.email,
			subject: `LayerRail Deploy Invoice ${invoice.number || ""} - ${amountFormatted}`,
			text: htmlContent,
			attachments,
		});
...
export const sendPaymentFailedEmail = async (
	...
	try {
		...
		await sendEmail({
			email: admin.email,
			subject: `Action required: LayerRail Deploy payment failed - ${amountFormatted}`,
			text: htmlContent,
		});
