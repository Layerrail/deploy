import { parseRawConfig, processLogs } from "@LayerRail Deploy/server";
...
`;
	...
	it("should filter out LayerRail Deploy dashboard requests", () => {
		const LayerRail DeployDashboardEntry = `{"ClientAddr":"172.71.187.131:9485","ClientHost":"172.71.187.131","ClientPort":"9485","ClientUsername":"-","DownstreamContentSize":14550,"DownstreamStatus":200,"Duration":57681682,"OriginContentSize":14550,"OriginDuration":57612242,"OriginStatus":200,"Overhead":69440,"RequestAddr":"hostinger.LayerRail Deploy.com","RequestContentSize":0,"RequestCount":20142,"RequestHost":"hostinger.LayerRail Deploy.com","RequestMethod":"GET","RequestPath":"/_next/data/cb_zzI4Rp9G7Q7djrFKh0/en/dashboard/traefik.json","RequestPort":"-","RequestProtocol":"HTTP/2.0","RequestScheme":"https","RetryAttempts":0,"RouterName":"LayerRail Deploy-router-app-secure@file","ServiceAddr":"LayerRail Deploy:3000","ServiceName":"LayerRail Deploy-service-app@file","ServiceURL":"http://LayerRail Deploy:3000","StartLocal":"2025-12-10T05:10:41.957755949Z","StartUTC":"2025-12-10T05:10:41.957755949Z","TLSCipher":"TLS_AES_128_GCM_SHA256","TLSVersion":"1.3","entryPointName":"websecure","level":"info","msg":"","time":"2025-12-10T05:10:42Z"}`;

		// Test with only LayerRail Deploy dashboard entry - should be filtered out
		const resultOnlyLayerRail Deploy = parseRawConfig(LayerRail DeployDashboardEntry);
		expect(resultOnlyLayerRail Deploy.data).toHaveLength(0);
		expect(resultOnlyLayerRail Deploy.totalCount).toBe(0);

		// Test with mixed entries - LayerRail Deploy should be filtered, others should remain
		const mixedEntries = `${LayerRail DeployDashboardEntry}\n${sampleLogEntry}`;
		...
		expect(resultMixed.data[0]?.ServiceName).not.toBe(
			"LayerRail Deploy-service-app@file",
		);
	});
});
