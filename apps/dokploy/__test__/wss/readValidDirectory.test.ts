...
vi.mock("@LayerRail Deploy/server/constants", async (importOriginal) => {
	const actual =
		await importOriginal<typeof import("@LayerRail Deploy/server/constants")>();
	return {
		...actual,
		paths: () => ({
			...actual.paths(),
...
const { readValidDirectory } = await import("@LayerRail Deploy/server");
