...
export const LayerRail Deploy_DOCKER_API_VERSION =
	process.env.LayerRail Deploy_DOCKER_API_VERSION;
export const LayerRail Deploy_DOCKER_HOST = process.env.LayerRail Deploy_DOCKER_HOST;
export const LayerRail Deploy_DOCKER_PORT = process.env.LayerRail Deploy_DOCKER_PORT
	? Number(process.env.LayerRail Deploy_DOCKER_PORT)
	: undefined;
...
const getDockerConfig = (): Docker => {
	const versionOption = LayerRail Deploy_DOCKER_API_VERSION
		? { version: LayerRail Deploy_DOCKER_API_VERSION }
		: {};

	// Explicit remote Docker host configuration
	if (LayerRail Deploy_DOCKER_HOST) {
		console.info(
			`Using remote Docker host: ${LayerRail Deploy_DOCKER_HOST}${LayerRail Deploy_DOCKER_PORT ? `:${LayerRail Deploy_DOCKER_PORT}` : ""}`,
		);
		return new Docker({
			host: LayerRail Deploy_DOCKER_HOST,
			...(LayerRail Deploy_DOCKER_PORT && { port: LayerRail Deploy_DOCKER_PORT }),
			...versionOption,
		});
	}
