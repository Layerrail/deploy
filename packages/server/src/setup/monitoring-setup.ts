...
import { getLayerRail DeployImageTag } from "../services/settings";
...
export const setupMonitoring = async (serverId: string) => {
	...
	const containerName = "LayerRail Deploy-monitoring";
	let imageName = "LayerRail Deploy/monitoring:latest";

	if (
		(getLayerRail DeployImageTag() !== "latest" ||
			process.env.NODE_ENV === "development") &&
		!IS_CLOUD
	) {
		imageName = "LayerRail Deploy/monitoring:canary";
	}
	...
	const settings: ContainerCreateOptions = {
		...
		HostConfig: {
			...
			Binds: [
				...
				"/proc:/host/proc:ro",
				"/etc/LayerRail Deploy/monitoring/monitoring.db:/app/monitoring.db",
			],
			NetworkMode: "host",
		},
		ExposedPorts: {
	...
	try {
		await execAsyncRemote(
			serverId,
			"mkdir -p /etc/LayerRail Deploy/monitoring && touch /etc/LayerRail Deploy/monitoring/monitoring.db",
		);
		if (serverId) {
