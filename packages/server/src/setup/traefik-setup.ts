...
export const initializeStandaloneTraefik = async ({
	...
	const containerName = "LayerRail Deploy-traefik";
	...
	const settings: ContainerCreateOptions = {
		...
		HostConfig: {
			...
			Binds: [
				...
				`${DYNAMIC_TRAEFIK_PATH}:/etc/LayerRail Deploy/traefik/dynamic`,
...
export const initializeTraefikService = async ({
	...
	const { MAIN_TRAEFIK_PATH, DYNAMIC_TRAEFIK_PATH } = paths(!!serverId);
	const imageName = `traefik:v${TRAEFIK_VERSION}`;
	const appName = "LayerRail Deploy-traefik";
			...
			ContainerSpec: {
				...
				Mounts: [
					...
					},
					{
						...
						Target: "/etc/LayerRail Deploy/traefik/dynamic",
					},
					{
						Type: "bind",
...
export const createDefaultServerTraefikConfig = () => {
	...
	const configFilePath = path.join(DYNAMIC_TRAEFIK_PATH, "LayerRail Deploy.yml");
	...
	const appName = "LayerRail Deploy";
			...
			...(process.env.NODE_ENV === "development"
				...
				: {
						...
						docker: {
							exposedByDefault: false,
							watch: true,
							network: "LayerRail Deploy-network",
						},
					}),
			file: {
				directory: "/etc/LayerRail Deploy/traefik/dynamic",
				watch: true,
			},
		},
		entryPoints: {
			...
			certificatesResolvers: {
				letsencrypt: {
					acme: {
						...
						storage: "/etc/LayerRail Deploy/traefik/dynamic/acme.json",
						httpChallenge: {
							entryPoint: "web",
						},
					},
