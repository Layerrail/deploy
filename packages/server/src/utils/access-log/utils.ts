	...
	const hourlyData = _(logString)
		...
		.map((entry) => {
			try {
				...
				if (log.ServiceName === "LayerRail Deploy-service-app@file") {
					return null;
				}
...
export function parseRawConfig(
	...
	try {
		...
		// Filter out LayerRail Deploy dashboard requests
		parsedLogs = parsedLogs.filter(
			(log) => log.ServiceName !== "LayerRail Deploy-service-app@file",
		);
