...
import { removeDirectoryIfExistsContent } from "@LayerRail Deploy/server/utils/filesystem/directory";
import {
	execAsync,
	execAsyncRemote,
	...
	try {
		...
		if (serverId) {
			...
			const command = `
			    ...
			    echo "Building on ${serverId ? "Build Server" : "LayerRail Deploy Server"}" >> ${logFilePath};
			`;
