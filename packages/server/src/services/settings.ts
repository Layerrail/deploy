...
/** Returns current LayerRail Deploy docker image tag or `latest` by default. */
export const getLayerRail DeployImageTag = () => {
	return process.env.RELEASE_TAG || "latest";
};

/** Returns LayerRail Deploy docker service image digest */
export const getServiceImageDigest = async () => {
	const { stdout } = await execAsync(
		"docker service inspect LayerRail Deploy --format '{{.Spec.TaskTemplate.ContainerSpec.Image}}'",
	);
...
export const getUpdateData = async (
	...
	try {
		const baseUrl =
			"https://hub.docker.com/v2/repositories/LayerRail Deploy/LayerRail Deploy/tags";
		...
		const currentImageTag = getLayerRail DeployImageTag();
...
export const reloadDockerResource = async (
	...
	if (resourceType === "service") {
		if (resourceName === "LayerRail Deploy") {
			const currentImageTag = getLayerRail DeployImageTag();
			...
			command = `docker service update --force --image LayerRail Deploy/LayerRail Deploy:${imageTag} ${resourceName}`;
		} else {
...
export const checkPortInUse = async (
	...
	try {
		...
		const dockerCommand = `docker ps -a --format '{{.Names}}' | grep -v '^LayerRail Deploy-traefik$' | while read name; do docker port "$name" 2>/dev/null | grep -q ':${port}' && echo "$name" && break; done || true`;
		...
		// LayerRail Deploy runs inside a container, so we spawn an ephemeral container
...
export const reconnectServicesToTraefik = async (serverId?: string) => {
	...
	for (const compose of composeResult) {
		commands += `docker network connect ${compose.appName} $(docker ps --filter "name=LayerRail Deploy-traefik" -q) >/dev/null 2>&1\n`;
	}
